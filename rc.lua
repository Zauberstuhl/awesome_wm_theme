...
bashets.register("/usr/local/bin/acpi_awesome_widget", 
{
  widget = batterystatus,
  update_time = 60,
  separator = '|',
  format = " | Battery:<span color=\"$1\" face=\"Monospace\">$2</span>"
})
bashets.start()
...
function currenttemp()
  ...
  return " <span color=\"" ..
    (tonumber(l) < 65 and "white" or "red") ..
    "\">" .. l .. " C°</span> | "
end

mytemp = wibox.widget.textbox( currenttemp() )
mytemp.timer = timer { timeout = 20 }
mytemp.timer:connect_signal("timeout", function () mytemp:set_markup( currenttemp() ) end)
mytemp.timer:start()
